    ...
    <div className="w-full h-full flex flex-col justify-between border-[1px] border-gray-300 rounded-[15px] shadow-md shadow-gray-200 p-4">
      ...
      <div className="flex w-full justify-between items-end">
        <div>
          ...
          <p className="font-bold text-lg">$ {data.price_usd}</p>
        </div>
        <span className={`rounded-full ${data.percent_change_1h>=0?'bg-emerald-100 text-green-500':'bg-red-300 text-red-500'} py-[1px] px-[5px]`}>{data.percent_change_1h>=0?`+${data.percent_change_1h}`:data.percent_change_1h}</span>
      </div>
    </div>
  );
};
